class shapeSketch {
  ...
  final static float n = new float []
    {
    {{0.3, 0.3, 0.3}, {1, 1, 1}},
    {{0.04, 1.7, 1.7}, {1, 1, 1}},
  ...
  void superShape  (float theta, float m, float n1, float n2, float n3) {
  ...
  void draw  () {
    ...
    mult = map(wFrequencyAmplitude[0], 0, 1000, 0, 3);
    //console.log("mult: ", mult);
    m1 = map(sin(millis() / 1000), -1, 1, 0, 5) + mult;
    m2 = map(sin(millis() / 3333), -1, 1, 0, 5);
    ...
    for (var i = 0; i < total + 1; i++) {
      ...
      var r2 = superShape(lat, m1, n[nRow][0][0], n[nRow][0][1], n[nRow][0][2]);
      //var r2 = supershape(lat, 2, 10, 10, 10);
      for (var j = 0; j < total + 1; j++) {
        var lon = map(j, 0, total, -PI, PI);
        var r1 = superShape(lon, m2, n[nRow][1][0], n[nRow][1][1], n[nRow][1][2]);
